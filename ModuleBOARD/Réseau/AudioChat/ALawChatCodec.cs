...
namespace NAudioDemo.NetworkChatDemo
{
    ...
    abstract class AcmChatCodec : INetworkChatCodec
    {
        ...
        public byte[] Encode(byte[] data, int offset, int length)
        {
            ...
            return ConvertEncode(encodeStream, data, offset, length, ref encodeSourceBytesLeftovers);
        }
        ...
        public byte[] Decode(byte[] data, int offset, int length)
        {
            ...
            return ConvertDecode(decodeStream, data, offset, length, ref decodeSourceBytesLeftovers);
        }

        private static byte[] ConvertEncode(AcmStream conversionStream, byte[] data, int offset, int length, ref int sourceBytesLeftovers)
        {
            ...
            byte[] encoded = new byte[1 + 2 + bytesConverted];
            Array.Copy(conversionStream.DestBuffer, 0, encoded, 3, bytesConverted);
            return encoded;
        }

        private static byte[] ConvertDecode(AcmStream conversionStream, byte[] data, int offset, int length, ref int sourceBytesLeftovers)
        {
            int bytesInSourceBuffer = length + sourceBytesLeftovers;
            Array.Copy(data, offset, conversionStream.SourceBuffer, sourceBytesLeftovers, length);
            int bytesConverted = conversionStream.Convert(bytesInSourceBuffer, out var sourceBytesConverted);
